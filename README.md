...
- Cancelable async navigation: a new resolve supersedes the in-flight one (`currentGuard`); `cancel()` aborts it; a history POP cancels it too. A superseded or cancelled `navigate()` promise rejects with a `NavigationCancelledError` — eagerly, without waiting for the aborted resolve to settle, so an `await` never hangs on a discarded navigation (a vetoed one resolves normally: a veto is the user saying no, not a failure). Fire-and-forget call sites attach a no-op catch (the react bindings' `Link` and setters do). Superseding or cancelling also aborts the chain's `AbortSignal`: guards (`beforeLoad` ctx) and view loaders (`ResolveViewContext`) receive it as `ctx.signal`, so their in-flight requests stop instead of only having results dropped; `preload` resolutions run under their own signal, aborted only by the concurrency bound
...
- Errors: `NativeRouterError`, `NotFoundError`, `RedirectLoopError`, `SearchError`, `NavigationCancelledError`
...
- **The schemas still validate:** the target's raw search is checked against every matched level's `search` schema before the snapshot is re-served — a rejected value abandons the fast path and the full resolution surfaces the `SearchError` through the normal error channel, so an invalid value never lands in the URL unchecked. An async search schema opts its chain out of the fast path entirely (its verdict cannot be awaited synchronously)
...
- **The contract cuts both ways — everything the level's resolution reads from the search must be declared:** a `beforeLoad` guard that reads search keys must see them listed or it will not re-run when they change
...
- A same-name param on both a parent and a child segment (`/users/:id/files/:id`): a schema-less child re-declaring the **same** raw value keeps the parent's coerced one (`{id: 7}` stays a number), and a child declaring its own `params` schema re-binds the key to its raw string so the schema coerces the URL-side value itself. A child segment with a **different** value is a new binding — the deeper string wins, exactly like the raw merge. Declaring a coercing schema on (or below) the deepest level that reads the param remains the fully general setup
